<?php
    ...
    session_start();
    $myusername=$_POST["yourUsername"];
    $ImgName = $_POST['imageFile'];
    //echo "<script>alert($ImgName);</script>";
    $sql1 = "SELECT id FROM lsb WHERE user = '$myusername' and imgName='$ImgName'";
    //echo $sql1;
    ...
    if($count == 1) {
        ...
        $command = "/usr/local/opt/python/libexec/bin/python decode_lsb.py $target_file";
        $output = shell_exec($command);
        //echo $output;
        $sql2 = "SELECT message FROM lsb WHERE user = '$myusername' and imgName='$ImgName'";
